      ...
      <div className="columns-1 gap-5 sm:columns-2 lg:columns-3">
        {items.map((item, index) => (
          <button
            ...
            className="group relative mb-5 block w-full break-inside-avoid overflow-hidden rounded-2xl border border-white/8 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-gold"
          >
            <ImageFrame
              src={item.src}
              alt={item.label}
              className={`w-full object-cover transition-transform duration-700 group-hover:scale-105 ${index === 0 || index === 4 ? "aspect-[3/4]" : "aspect-[4/3]"}`}
            />
            <span className="absolute inset-0 flex flex-col justify-end bg-gradient-to-t from-ink/80 via-transparent to-transparent p-5 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
              <span className="text-xs uppercase tracking-luxe text-gold">
                {item.kind}
              </span>
              <span className="mt-1 font-heading text-xl text-cream">
                {item.label}
              </span>
            </span>
          </button>
            ...
            >
              ...
              <div className="mt-4 flex items-center justify-between px-2">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => move(-1)}
                    className="rounded-full border border-white/15 p-3 text-cream hover:border-gold hover:text-gold"
